...
%                'YRange' : y range of the figure that is occupied by the signal p300
...
opts = hlp_varargin2struct(varargin, ...
    {'yrange','YRange'}, [0.05 0.95], ...       % y range of the figure occupied by the signal p300
        ...
        if opts.show_events && ~isempty(old.event)
            ...
            if ~isempty(evtindices)
                ...
                for ty = visible_types(:)'
                    % p300 line instances in the right color
